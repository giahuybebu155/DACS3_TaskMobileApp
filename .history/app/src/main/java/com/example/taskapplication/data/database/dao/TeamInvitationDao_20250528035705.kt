...
interface TeamInvitationDao {
     ...
     */
    @Query("SELECT * FROM team_invitations WHERE teamId = :teamId AND email = :email AND syncStatus != 'pending_delete' ORDER BY createdAt DESC LIMIT 1")
    suspend fun getInvitationByTeamAndEmail(teamId: String, email: String): TeamInvitationEntity?
