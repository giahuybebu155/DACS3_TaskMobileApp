...
class ConnectionChecker @Inject constructor(
    ...
    private suspend fun checkServerConnection(host: String, port: String): Boolean {
        return withContext(Dispatchers.IO) {
            try {
                Log.d(TAG, "🔌 [SOCKET_CHECK] Attempting to connect to $host:$port")
                Log.d(TAG, "🔌 [SOCKET_CHECK] Creating socket...")

                val socket = java.net.Socket()
                val socketAddress = java.net.InetSocketAddress(host, port.toInt())

                Log.d(TAG, "🔌 [SOCKET_CHECK] Socket address: $socketAddress")
                Log.d(TAG, "🔌 [SOCKET_CHECK] Connecting with 1 second timeout (fast fail)...")

                val startTime = System.currentTimeMillis()
                socket.connect(socketAddress, 1000) // 1 second timeout - fast fail to prevent ANR
                val endTime = System.currentTimeMillis()

                Log.d(TAG, "✅ [SOCKET_CHECK] Connected successfully to $host:$port in ${endTime - startTime}ms")
                socket.close()
                Log.d(TAG, "🔌 [SOCKET_CHECK] Socket closed")
                true
            } catch (e: Exception) {
                Log.e(TAG, "❌ [SOCKET_CHECK] Failed to connect to $host:$port")
                Log.e(TAG, "❌ [SOCKET_CHECK] Exception type: ${e.javaClass.simpleName}")
                Log.e(TAG, "❌ [SOCKET_CHECK] Exception message: ${e.message}")
                when (e) {
                    is java.net.ConnectException -> {
                        Log.e(TAG, "❌ [SOCKET_CHECK] Connection refused - server not running or port blocked")
                    }
                    is java.net.SocketTimeoutException -> {
                        Log.e(TAG, "❌ [SOCKET_CHECK] Connection timeout - server not responding")
                    }
                    is java.net.UnknownHostException -> {
                        Log.e(TAG, "❌ [SOCKET_CHECK] Unknown host - DNS resolution failed")
                    }
                    else -> {
                        Log.e(TAG, "❌ [SOCKET_CHECK] Other network error")
                    }
                }
                false
            }
