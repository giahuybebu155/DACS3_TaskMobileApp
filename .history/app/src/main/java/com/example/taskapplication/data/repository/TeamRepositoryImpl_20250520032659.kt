                    ...
                    withContext(Dispatchers.IO) {
                        ...
                        for (serverTeam in serverTeams) {
                            ...
                            if (localTeam == null) {
                                ...
                                // Thêm người dùng hiện tại vào nhóm nếu chưa là thành viên
                                val currentUserId = dataStoreManager.getCurrentUserId()
                                if (currentUserId != null) {
                                    val isAlreadyMember = teamMemberDao.isUserMemberOfTeam(newTeam.id, currentUserId)
                                    if (!isAlreadyMember) {
                                        Log.d(TAG, "🔄 [THEO DÕI] Thêm người dùng hiện tại vào nhóm: ${newTeam.name} (ID: ${newTeam.id})")
                                        val teamMemberEntity = TeamMemberEntity(
                                            id = UUID.randomUUID().toString(),
                                            teamId = newTeam.id,
                                            userId = currentUserId,
                                            role = "member", // Mặc định là member
                                            joinedAt = System.currentTimeMillis(),
                                            invitedBy = newTeam.ownerId ?: currentUserId,
                                            serverId = null,
                                            syncStatus = "synced",
                                            lastModified = System.currentTimeMillis(),
                                            createdAt = System.currentTimeMillis()
