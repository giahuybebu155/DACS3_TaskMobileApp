...
class TeamRepositoryImpl @Inject constructor(
    ...
    override suspend fun createTeam(team: Team): Result<Team> {
        try {
            ...
            val teamMemberEntity = TeamMemberEntity(
                ...
                role = "owner", // Người tạo team mặc định là owner
            ...
            if (connectionChecker.isNetworkAvailable()) {
                try {
                    ...
                    if (response.isSuccessful && response.body() != null) {
                        ...
                        // Đồng bộ thành viên nhóm (admin)
                        try {
                            Log.d(TAG, "🔄 [THEO DÕI] Đang gửi thành viên nhóm (admin) lên server")
                            // Tạo request body thay vì sử dụng query parameters
                            val memberRequest = com.example.taskapplication.data.api.request.TeamMemberRequest(
                                userId = currentUserId,
                                role = "owner"  // Thay đổi từ "admin" thành "owner" vì có thể API chỉ chấp nhận "owner" hoặc "member"
                            )
    ...
    override suspend fun inviteUserToTeam(teamId: String, userEmail: String): Result<com.example.taskapplication.domain.model.TeamMember> {
        try {
            ...
            // Kiểm tra xem người dùng hiện tại có phải là owner hoặc admin của team hay không
            val isAdmin = teamMemberDao.isUserAdminOfTeam(teamId, currentUserId)
            if (!isAdmin) {
                Log.e(TAG, "❌ [THEO DÕI] Người dùng không phải là owner hoặc admin của nhóm")
                return Result.failure(IOException("Bạn không có quyền mời người dùng vào nhóm này"))
            }
            Log.d(TAG, "✅ [THEO DÕI] Người dùng là owner hoặc admin của nhóm")
            ...
            if (connectionChecker.isNetworkAvailable()) {
                try {
                    ...
                    val invitationRequest = com.example.taskapplication.data.api.request.TeamInvitationRequest(
                        email = userEmail,
                        role = "member",
                        userId = user.id  // Thêm user_id vào request
                    )
    ...
    override suspend fun changeUserRole(teamId: String, userId: String, newRole: String): Result<com.example.taskapplication.domain.model.TeamMember> {
        try {
            ...
            // Kiểm tra xem có phải là admin hoặc owner cuối cùng không
            if ((oldRole == "admin" || oldRole == "owner") && (newRole != "admin" && newRole != "owner")) {
                val adminCount = teamMemberDao.countAdminsInTeam(teamId)
                if (adminCount <= 1) {
                    return Result.failure(IOException("Không thể hạ cấp admin/owner cuối cùng của nhóm"))
                }
            }
            ...
            try {
                ...
                if (response.isSuccessful && response.body() != null) {
                    ...
                    withContext(Dispatchers.IO) {
                        ...
                        // Tạo map các nhóm cục bộ theo serverId
                        val localTeamMap = localTeams.associateBy { team -> team.serverId }

                        // Lấy ID người dùng hiện tại
                        val currentUserId = dataStoreManager.getCurrentUserId()

                        // Tạo danh sách các ID nhóm từ server để theo dõi những nhóm nào đã được xử lý
                        val processedServerTeamIds = mutableSetOf<String>()

                        // Xử lý từng nhóm từ server
                        for (serverTeam in serverTeams) {
                            processedServerTeamIds.add(serverTeam.id)

                            // Kiểm tra xem nhóm đã tồn tại trong cơ sở dữ liệu cục bộ chưa
                            val localTeam = localTeamMap[serverTeam.id]

                            if (localTeam == null) {
                                // Nhóm chưa tồn tại, thêm mới
                                Log.d(TAG, "🔄 [THEO DÕI] Thêm nhóm mới từ server: ${serverTeam.name} (ID: ${serverTeam.id})")

                                // Kiểm tra xem nhóm đã tồn tại với ID khác không (dựa trên tên)
                                val existingTeamWithSameName = localTeams.find { it.name == serverTeam.name }

                                if (existingTeamWithSameName != null && existingTeamWithSameName.serverId == null) {
                                    // Cập nhật nhóm hiện có với ID từ server
                                    Log.d(TAG, "🔄 [THEO DÕI] Tìm thấy nhóm cục bộ với cùng tên, cập nhật với ID từ server")
                                    val updatedTeam = existingTeamWithSameName.copy(
                                        serverId = serverTeam.id,
                                    ...
                                    teamDao.updateTeam(updatedTeam)
                                    Log.d(TAG, "✅ [THEO DÕI] Đã cập nhật nhóm cục bộ với ID từ server")
                                } else {
                                    // Thêm nhóm mới
                            ...
                            } else if (localTeam.syncStatus != "pending_delete") {
                                // Nhóm đã tồn tại, cập nhật thông tin
                                Log.d(TAG, "🔄 [THEO DÕI] Cập nhật nhóm từ server: ${serverTeam.name} (ID: ${serverTeam.id})")
                                val updatedTeam = localTeam.copy(
                        ...
                        // Xóa các nhóm cục bộ không còn tồn tại trên server (trừ các nhóm đang chờ tạo mới)
                        val localTeamsToDelete = localTeams.filter {
                            it.serverId != null &&
                            it.serverId !in processedServerTeamIds &&
                            it.syncStatus != "pending_create"
                        }

                        if (localTeamsToDelete.isNotEmpty()) {
                            Log.d(TAG, "🔄 [THEO DÕI] Xóa ${localTeamsToDelete.size} nhóm cục bộ không còn tồn tại trên server")

                            for (teamToDelete in localTeamsToDelete) {
                                Log.d(TAG, "🔄 [THEO DÕI] Xóa nhóm cục bộ: ${teamToDelete.name} (ID: ${teamToDelete.id}, ServerID: ${teamToDelete.serverId})")
                                teamDao.deleteTeam(teamToDelete.id)
                                Log.d(TAG, "✅ [THEO DÕI] Đã xóa nhóm khỏi cơ sở dữ liệu cục bộ")
                            }
                        }
                    }
