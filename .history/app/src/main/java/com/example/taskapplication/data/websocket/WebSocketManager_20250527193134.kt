...
class WebSocketManager @Inject constructor(
    ...
    /**
     * Kết nối đến WebSocket server (chỉ cần authToken - RECOMMENDED)
     * Server sẽ tự động subscribe user vào tất cả teams của họ
     * @param authToken Token xác thực
     */
    fun connect(authToken: String) {
        Log.d(TAG, "🔌 [CONNECT] ===== USER-BASED WEBSOCKET CONNECTION =====")
        Log.d(TAG, "🔌 [CONNECT] Auth Token: ${authToken.take(10)}...${authToken.takeLast(5)}")
        Log.d(TAG, "👤 [CONNECT] User-based connection (no teamId required)")
        Log.d(TAG, "🔌 [CONNECT] Current connection state: ${_connectionState.value}")

        // Nếu đã kết nối với cùng token, không cần kết nối lại
        if (_connectionState.value == ConnectionState.CONNECTED && lastAuthToken == authToken) {
            Log.d(TAG, "⚡ [CONNECT] WebSocket already connected with same token - SKIPPING")
            return
        }
        ...
        lastTeamId = "" // Not needed for user-based connection
        ...
        // ✅ LARAVEL REVERB - TRY MULTIPLE ENDPOINTS
        val wsUrl = ReverbConfig.getWebSocketUrl(authToken)
        ReverbLogger.logConnection(wsUrl)
        Log.d(TAG, "🌐 [CONNECT] WebSocket URL: $wsUrl")
        Log.d(TAG, "🎯 [CONNECT] Laravel Reverb endpoint (Pusher protocol)")
        Log.d(TAG, "🔧 [CONNECT] If 404, will try alternative endpoints")
        ...
        return object : WebSocketListener() {
            override fun onOpen(webSocket: WebSocket, response: Response) {
                ...
                if (teamId != null) {
                    ...
                    Log.d(TAG, "👤 [WS_OPEN] User-based connection - server will auto-subscribe to user's teams")
                }
                ...
                val connectionType = if (teamId != null) "team-specific ($teamId)" else "user-based"
                Log.d(TAG, "🎯 [WS_OPEN] WebSocket connected with $connectionType subscription")
    ...
    private suspend fun processMessage(text: String) {
        try {
            val reverbEvent = parseReverbEvent(text)
            if (reverbEvent == null) {
                Log.e(TAG, "❌ [PARSE] Failed to parse Reverb event")
                return
            }

            ReverbLogger.logEvent(reverbEvent.event, reverbEvent.channel, text)

            // Handle Pusher/Reverb system events first
            when (reverbEvent.event) {
                "pusher:connection_established" -> {
                    Log.d(TAG, "✅ [REVERB] Connection established")
                    handleConnectionEstablished()
                    return
                }

                "pusher:subscription_succeeded" -> {
                    Log.d(TAG, "✅ [REVERB] Subscribed to: ${reverbEvent.channel}")
                    handleSubscriptionSucceeded(reverbEvent.channel)
                    return
                }

                "pusher:subscription_error" -> {
                    Log.e(TAG, "❌ [REVERB] Subscription failed: ${reverbEvent.channel}")
                    handleSubscriptionError(reverbEvent.channel, reverbEvent.data)
                    return
                }
            }

            // Handle application events
            val eventName = reverbEvent.event
            val data = reverbEvent.data
            ...
            when (eventName) {
                ...
                "message-reaction-updated" -> {
                    ...
                    val userId = data?.optLong("user_id") ?: return
                    val reaction = data?.optString("reaction") ?: return
                    val timestamp = data?.optLong("created_at", System.currentTimeMillis()) ?: System.currentTimeMillis()
                ...
                "message-updated" -> {
                    ...
                    val content = data?.optString("message") ?: return
                    val updatedAt = data?.optLong("updated_at", System.currentTimeMillis()) ?: System.currentTimeMillis()
                ...
                "message-deleted" -> {
                    val messageId = data?.optLong("id") ?: return
                    val deletedAt = data?.optLong("deleted_at", System.currentTimeMillis()) ?: System.currentTimeMillis()
                ...
                "team.member.added" -> {
                    ...
                    val role = data?.optString("role", "member") ?: "member"
                ...
                "team.member.updated", "team.member.role.changed" -> {
                    ...
                    val teamId = data?.optLong("team_id")?.toString() ?: data?.optString("team_id") ?: return
                    val userId = data?.optLong("user_id")?.toString() ?: data?.optString("user_id") ?: return
                    val newRole = data?.optString("role", "member") ?: "member"
    ...
    private fun reconnect() {
        ...
        if (newToken != null) {
            Log.d(TAG, "🔄 [RECONNECT] Using token from AuthManager: ${newToken.take(10)}...")
            lastAuthToken = newToken
            connect(newToken) // Use user-based connection (RECOMMENDED)
        } else if (lastAuthToken.isNotEmpty()) {
            Log.d(TAG, "🔄 [RECONNECT] Using cached token: ${lastAuthToken.take(10)}...")
            connect(lastAuthToken) // Use user-based connection (RECOMMENDED)
        } else {
            Log.e(TAG, "❌ [RECONNECT] Cannot reconnect: no auth token available")
            Log.e(TAG, "❌ [RECONNECT] AuthManager token: ${if (newToken != null) "available" else "null"}")
            Log.e(TAG, "❌ [RECONNECT] Cached token: ${if (lastAuthToken.isNotEmpty()) "available" else "empty"}")
            _connectionState.value = ConnectionState.ERROR
        }
    }
    ...
    /**
     * Parse Reverb event from raw message
     */
    private fun parseReverbEvent(message: String): ReverbEvent? {
        return try {
            ...
            Log.e(TAG, "Failed to parse Reverb event: ${e.message}")
            null
        }
    }
    ...
    private fun handleConnectionEstablished() {
        Log.d(TAG, "🎉 [REVERB] Connection established - auto-subscribing to user channels")
        // Auto-subscribe to user's teams will be handled by server
    }

    /**
     * Handle subscription succeeded event
     */
    private fun handleSubscriptionSucceeded(channel: String) {
        ReverbLogger.logSubscription(channel, true)
    }
                ...
                try {
                    ...
                    val testSocket = client.newWebSocket(request, object : WebSocketListener() {
                        override fun onOpen(webSocket: WebSocket, response: Response) {
                            ...
                            Log.d(TAG, "🎯 [ALT_ENDPOINTS] Using working endpoint for reconnection")
                            // Reconnect with working endpoint
                            scheduleReconnect()
                        }
