...
class WebSocketManager @Inject constructor(
    ...
    fun connect(authToken: String, teamId: String) {
        ...
        // CRITICAL FIX: Save connection info FIRST for reconnection
    ...
    private fun reconnect() {
        if (lastAuthToken.isNotEmpty() && lastTeamId.isNotEmpty()) {
            ...
            Log.d(TAG, "🔍 [RECONNECT] Checking stored credentials:")
            Log.d(TAG, "   - Token available: ${newToken != null}")
            Log.d(TAG, "   - TeamId available: ${!currentTeamId.isNullOrEmpty()}")
            Log.d(TAG, "   - TeamId value: $currentTeamId")

            if (newToken != null && !currentTeamId.isNullOrEmpty()) {
                Log.d(TAG, "✅ [RECONNECT] Found valid credentials - reconnecting")
                lastAuthToken = newToken
                lastTeamId = currentTeamId
                connect(newToken, currentTeamId)
            } else {
                Log.e(TAG, "❌ [RECONNECT] Cannot reconnect: missing connection information")
                Log.e(TAG, "   - Token: ${if (newToken != null) "AVAILABLE" else "NULL"}")
                Log.e(TAG, "   - TeamId: ${if (!currentTeamId.isNullOrEmpty()) "AVAILABLE" else "NULL/EMPTY"}")

                // STOP reconnection attempts - set to DISCONNECTED instead of ERROR
                _connectionState.value = ConnectionState.DISCONNECTED

                // Cancel any pending reconnect jobs
                reconnectJob?.cancel()
                Log.d(TAG, "🚫 [RECONNECT] Stopped reconnection attempts due to missing credentials")
            }
        }
    }
