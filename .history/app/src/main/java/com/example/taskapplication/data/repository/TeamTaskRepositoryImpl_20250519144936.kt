    ...
    override suspend fun syncTasks(): Result<Unit> {
        ...
        try {
            ...
            if (response.isSuccessful && response.body() != null) {
                ...
                // Xử lý các task mới
                if (syncResponse.data.teamTasks != null) {
                    val createdTasks = syncResponse.data.teamTasks["created"] ?: emptyList()
                    for (task in createdTasks) {
                        teamTaskDao.insertTask(task.toEntity())
                    }

                    // Xử lý các task đã cập nhật
                    val updatedTasks = syncResponse.data.teamTasks["updated"] ?: emptyList()
                    for (task in updatedTasks) {
                        teamTaskDao.updateTask(task.toEntity())
                    }

                    // Xử lý các task đã xóa
                    val deletedTasks = syncResponse.data.teamTasks["deleted"] ?: emptyList()
                    for (taskId in deletedTasks) {
                        val task = teamTaskDao.getTaskByServerId(taskId.id)
                        if (task != null) {
                            teamTaskDao.deleteTask(task)
                        }
                    }
    ...
    override suspend fun syncTasksByTeam(teamId: String): Result<Unit> {
        ...
        try {
            ...
            if (response.isSuccessful && response.body() != null) {
                ...
                // Xử lý các task mới
                for (task in syncResponse.data.created) {
                    teamTaskDao.insertTask(task.toEntity())
                }

                // Xử lý các task đã cập nhật
                for (task in syncResponse.data.updated) {
                    teamTaskDao.updateTask(task.toEntity())
                }

                // Xử lý các task đã xóa
                for (taskId in syncResponse.data.deleted) {
                    val task = teamTaskDao.getTaskByServerId(taskId)
                    if (task != null) {
                        teamTaskDao.deleteTask(task)
                    }
                }
