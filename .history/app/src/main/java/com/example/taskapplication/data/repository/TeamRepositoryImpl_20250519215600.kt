...
class TeamRepositoryImpl @Inject constructor(
    ...
    override suspend fun inviteUserToTeam(teamId: String, userEmail: String): Result<com.example.taskapplication.domain.model.TeamMember> {
        try {
            ...
            // Sử dụng teamInvitationDao đã được inject
            val teamInvitationDao = com.example.taskapplication.data.database.AppDatabase.getInstance(applicationContext)
                .teamInvitationDao()
            teamInvitationDao.insertInvitation(invitationEntity)

            // Nếu có kết nối mạng, đồng bộ lên server
            if (connectionChecker.isNetworkAvailable()) {
                try {
                    Log.d(TAG, "Đang gửi lời mời lên server: teamId=$teamId, userId=${user.id}, role=member")
                    val response = apiService.inviteUserToTeam(teamId, user.id, "member")

                    if (response.isSuccessful && response.body() != null) {
                        Log.d(TAG, "Gửi lời mời thành công: ${response.body()}")
                        ...
                        Log.e(TAG, "Lỗi khi gửi lời mời lên server: ${response.code()} - ${response.message()}")
                    }
                } catch (e: Exception) {
