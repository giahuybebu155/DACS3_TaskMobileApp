...
class WebSocketManager @Inject constructor(
    ...
    fun connect(authToken: String, teamId: String) {
        ...
        if (_connectionState.value == ConnectionState.CONNECTED ||
            ...
            Log.d(TAG, "WebSocket already connected or connecting, skipping connection request")
            return
        }
    ...
    private suspend fun processMessage(text: String) {
        try {
            ...
            when (eventName) {
                ...
                "team.invitation.created", "team-invitation" -> {
                    Log.d(TAG, "Received team invitation event: $data")
                    val teamId = data?.optLong("team_id")?.toString() ?: return
                    val inviterId = data.optLong("inviter_id")?.toString() ?: return
                    ...
                    val workManager = androidx.work.WorkManager.getInstance(androidx.work.impl.utils.WorkManagerImpl.getInstance().applicationContext)
    ...
    private fun reconnect() {
        if (lastAuthToken.isNotEmpty() && lastTeamId.isNotEmpty()) {
            ...
            // Try to get new token from AuthManager
            val newToken = authManager.getAuthToken()
            if (newToken != null && lastTeamId.isNotEmpty()) {
                Log.d(TAG, "Reconnecting with new token from AuthManager")
                lastAuthToken = newToken
                connect(newToken, lastTeamId)
            }
        }
    }
