...
class TeamInvitationRepositoryImpl @Inject constructor(
                ...
                try {
                    ...
                    if (response.isSuccessful) {
                        ...
                        responseBody?.let { body ->
                            ...
                            // Trigger đồng bộ dữ liệu team ngay sau khi accept
            ...
            try {
                ...
                if (!email.isNullOrEmpty()) {
                    ...
                    try {
                        ...
                        if (response.isSuccessful && response.body() != null) {
                            try {
                                val userInvitationsResponse = response.body()!!
                                val userInvitations = userInvitationsResponse.data
    ...
    private fun triggerTeamDataSync(teamId: String) {
        try {
            ...
            // 2. Đồng bộ tin nhắn team
            val messageSyncWork = androidx.work.OneTimeWorkRequestBuilder<com.example.taskapplication.workers.MessageSyncWorker>()
                .setExpedited(androidx.work.OutOfQuotaPolicy.RUN_AS_NON_EXPEDITED_WORK_REQUEST)
                .setInputData(androidx.work.workDataOf("team_id" to teamId))
                .build()
            workManager.enqueue(messageSyncWork)
            Log.d(TAG, "✅ [TEAM_SYNC] Đã enqueue MessageSyncWorker cho team: $teamId")

            // 3. Đồng bộ tasks team
            val taskSyncWork = androidx.work.OneTimeWorkRequestBuilder<com.example.taskapplication.workers.TaskSyncWorker>()
                .setExpedited(androidx.work.OutOfQuotaPolicy.RUN_AS_NON_EXPEDITED_WORK_REQUEST)
                .setInputData(androidx.work.workDataOf("team_id" to teamId))
                .build()
            workManager.enqueue(taskSyncWork)
            Log.d(TAG, "✅ [TEAM_SYNC] Đã enqueue TaskSyncWorker cho team: $teamId")

            // 4. Đồng bộ documents team (nếu có)
            try {
                val documentSyncWork = androidx.work.OneTimeWorkRequestBuilder<com.example.taskapplication.workers.DocumentSyncWorker>()
                    .setExpedited(androidx.work.OutOfQuotaPolicy.RUN_AS_NON_EXPEDITED_WORK_REQUEST)
                    .setInputData(androidx.work.workDataOf("team_id" to teamId))
                    .build()
                workManager.enqueue(documentSyncWork)
                Log.d(TAG, "✅ [TEAM_SYNC] Đã enqueue DocumentSyncWorker cho team: $teamId")
            } catch (e: Exception) {
                Log.w(TAG, "⚠️ [TEAM_SYNC] DocumentSyncWorker không có sẵn, bỏ qua: ${e.message}")
            }
