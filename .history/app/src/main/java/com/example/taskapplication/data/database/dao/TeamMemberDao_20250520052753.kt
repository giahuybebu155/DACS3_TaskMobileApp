...
interface TeamMemberDao {
    ...
    @Query("SELECT EXISTS(SELECT 1 FROM team_members WHERE teamId = :teamId AND userId = :userId AND (role = 'admin' OR role = 'owner'))")
    ...
    @Query("SELECT COUNT(*) FROM team_members WHERE teamId = :teamId AND (role = 'admin' OR role = 'owner')")
