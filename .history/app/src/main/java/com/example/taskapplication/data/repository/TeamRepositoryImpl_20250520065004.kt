...
class TeamRepositoryImpl @Inject constructor(
            ...
            if (connectionChecker.isNetworkAvailable()) {
                try {
                    ...
                    if (syncedTeam.serverId == null) {
                        ...
                        // Sử dụng serverId của nhóm
                        val response = apiService.sendInvitationByUuid(updatedTeam.serverId!!, invitationRequest)
    ...
    private suspend fun handleInvitationResponse(
        response: retrofit2.Response<com.example.taskapplication.data.api.response.TeamMemberResponse>,
            ...
            for (member in membersToCreate) {
                try {
                    ...
                    val response = apiService.sendInvitationByUuid(
                        teamUuid = team.serverId!!, // Sử dụng serverId thay vì UUID cục bộ
                ...
                try {
                    if (member.serverId != null) {
                        ...
                        val response = apiService.updateTeamMember(
                            teamId = team.serverId!!, // Sử dụng serverId thay vì UUID cục bộ
