...
class AuthRepositoryImpl @Inject constructor(
    ...
    private val webSocketManagerProvider: javax.inject.Provider<com.example.taskapplication.data.websocket.WebSocketManager>
) : AuthRepository {
    ...
    override suspend fun login(email: String, password: String): Result<User> {
        return try {
            ...
            if (response.isSuccessful && response.body() != null) {
                ...
                try {
                    webSocketManagerProvider.get().connect(authResponse.token)
    ...
    override suspend fun register(name: String, email: String, password: String): Result<User> {
        return try {
            ...
            if (response.isSuccessful && response.body() != null) {
                ...
                // 🚀 AUTO-CONNECT WEBSOCKET AFTER REGISTER
                Log.d(TAG, "🔌 [REGISTER] Auto-connecting WebSocket after successful register")
                try {
                    webSocketManagerProvider.get().connect(authResponse.token)
                    Log.d(TAG, "✅ [REGISTER] WebSocket connection initiated")
                } catch (e: Exception) {
                    Log.e(TAG, "❌ [REGISTER] Failed to connect WebSocket: ${e.message}", e)
                    // Don't fail register if WebSocket fails
                }
    ...
    override suspend fun loginWithGoogle(idToken: String): Result<User> {
        return try {
            ...
            if (response.isSuccessful && response.body() != null) {
                ...
                // 🚀 AUTO-CONNECT WEBSOCKET AFTER GOOGLE LOGIN
                Log.d(TAG, "🔌 [GOOGLE_LOGIN] Auto-connecting WebSocket after successful Google login")
                try {
                    webSocketManagerProvider.get().connect(authResponse.token)
                    Log.d(TAG, "✅ [GOOGLE_LOGIN] WebSocket connection initiated")
                } catch (e: Exception) {
                    Log.e(TAG, "❌ [GOOGLE_LOGIN] Failed to connect WebSocket: ${e.message}", e)
                    // Don't fail login if WebSocket fails
                }
    ...
    override suspend fun logout(): Result<Unit> {
        return try {
            ...
            Log.d(TAG, "🔌 [LOGOUT] Disconnecting WebSocket after logout")
            try {
                webSocketManagerProvider.get().disconnect()
                Log.d(TAG, "✅ [LOGOUT] WebSocket disconnected")
            } catch (e: Exception) {
                Log.e(TAG, "❌ [LOGOUT] Failed to disconnect WebSocket: ${e.message}", e)
            ...
            // 🔌 DISCONNECT WEBSOCKET AFTER LOGOUT (fallback)
            try {
                webSocketManagerProvider.get().disconnect()
                Log.d(TAG, "✅ [LOGOUT_FALLBACK] WebSocket disconnected")
            } catch (e: Exception) {
                Log.e(TAG, "❌ [LOGOUT_FALLBACK] Failed to disconnect WebSocket: ${e.message}", e)
            }
