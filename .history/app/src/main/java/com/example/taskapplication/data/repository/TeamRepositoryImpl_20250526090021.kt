    ...
    override suspend fun syncTeams(): Result<Unit> {
        ...
        try {
            ...
            try {
                ...
                Log.d(TAG, "🌐 [SYNC] Timeout settings: connect=5s, read=10s, write=5s")
        ...
        try {
            ...
            for (member in membersToCreate) {
                try {
                    ...
                    // Sử dụng endpoint phù hợp dựa trên kiểu serverId
                    val response = if (isNumeric) {
                        Log.d(TAG, "🔄 [THEO DÕI] ServerId là số (${team.serverId}), sử dụng endpoint sendInvitation")
                        apiService.sendInvitation(
                            teamId = team.serverId!!,
                            request = invitationRequest
                        )
                    } else {
                        Log.d(TAG, "🔄 [THEO DÕI] ServerId không phải là số (${team.serverId}), sử dụng endpoint sendInvitationByUuid")
                        apiService.sendInvitationByUuid(
                            teamUuid = team.serverId!!,
                            request = invitationRequest
                        )
                    }
