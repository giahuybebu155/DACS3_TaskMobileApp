...
class WebSocketManager @Inject constructor(
    ...
    fun connect(authToken: String) {
        ...
        val possibleUrls = listOf(
            "ws://10.0.2.2:8080/ws?token=$authToken",
            "ws://10.0.2.2:8080/websocket?token=$authToken",
            "ws://10.0.2.2:8080/socket?token=$authToken",
            "ws://10.0.2.2:8080/api/ws?token=$authToken",
            "ws://10.0.2.2:8080/chat?token=$authToken",
            // Test with echo server for debugging
            "ws://echo.websocket.org"
        )
        // For debugging: use echo server first
        val wsUrl = "ws://echo.websocket.org" // Test with echo server
        // val wsUrl = possibleUrls[0] // Use this for real server
        Log.d(TAG, "🔍 [CONNECT] Trying WebSocket URL: $wsUrl")
        Log.d(TAG, "📋 [CONNECT] Available URLs: ${possibleUrls.joinToString(", ")}")
