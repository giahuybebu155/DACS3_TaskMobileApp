        ...
        try {
            ...
            if (response.isSuccessful && response.body() != null) {
                ...
                for (memberResponse in teamMembers) {
                    ...
                    Log.d(TAG, "🔍 [DEBUG ROLES] Existing member role: ${existingMember?.role}, Server role: ${memberResponse.role}")

                    if (existingMember != null) {
                        val updatedMember = memberEntity.copy(id = existingMember.id)
                        teamMemberDao.updateTeamMember(updatedMember)
                        ...
                        Log.d(TAG, "🔍 [DEBUG ROLES] Inserted new member: userId=${memberEntity.userId}, role=${memberEntity.role}")
                    }
                }
