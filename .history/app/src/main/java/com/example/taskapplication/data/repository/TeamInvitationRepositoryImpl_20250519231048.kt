...
class TeamInvitationRepositoryImpl @Inject constructor(
    ...
    override suspend fun syncInvitations(): Result<Unit> {
        ...
        try {
            ...
            Log.d(TAG, "Lấy danh sách nhóm để đồng bộ lời mời")
            try {
                val teamsResponse = apiService.getUserTeams()
                val teams = if (teamsResponse.isSuccessful && teamsResponse.body() != null) {
                    teamsResponse.body()!!.map { it.id.toString() }
                } else {
                    // Fallback to a default team ID if we can't get the teams
                    val currentTeamId = dataStoreManager.getCurrentTeamId()
                    if (currentTeamId != null) listOf(currentTeamId) else listOf("1")
                }

                Log.d(TAG, "Đồng bộ lời mời cho ${teams.size} nhóm: $teams")

                // Lấy lời mời cho mỗi nhóm
                for (teamId in teams) {
                    try {
                        Log.d(TAG, "Lấy lời mời cho nhóm $teamId")
                        val response = apiService.getTeamInvitations(teamId)
                        Log.d(TAG, "Kết quả lấy lời mời: ${response.code()}")

                        if (response.isSuccessful && response.body() != null) {
                            val serverInvitations = response.body()!!
                            Log.d(TAG, "Nhận được ${serverInvitations.size} lời mời từ server cho nhóm $teamId")
                            ...
                            for (serverInvitation in serverInvitations) {
                                ...
                                Log.d(TAG, "Kiểm tra lời mời ${serverInvitation.id}: ${if (existingInvitation != null) "đã tồn tại" else "mới"}")
                                ...
                                if (existingInvitation == null) {
                                    ...
                                    Log.d(TAG, "Đã thêm lời mời mới vào cơ sở dữ liệu cục bộ: ${serverInvitation.id}")
                                    ...
                                    teamInvitationDao.updateInvitation(updatedInvitation)
                                    Log.d(TAG, "Đã cập nhật lời mời hiện có: ${serverInvitation.id}")
                                }
                            }
                        } else {
                            val errorBody = response.errorBody()?.string()
                            Log.e(TAG, "Lỗi khi lấy lời mời từ server: ${response.code()}, Error: $errorBody")
                        }
                    } catch (e: Exception) {
                        Log.e(TAG, "Lỗi khi lấy lời mời cho nhóm $teamId", e)
                    }
                }
            } catch (e: Exception) {
                Log.e(TAG, "Lỗi khi lấy danh sách nhóm", e)
            }
            ...
            try {
                ...
                if (!email.isNullOrEmpty()) {
                    Log.d(TAG, "Lấy lời mời gửi đến người dùng với email: $email")
                    val response = apiService.getUserInvitations()
                    Log.d(TAG, "Kết quả lấy lời mời người dùng: ${response.code()}")

                    if (response.isSuccessful && response.body() != null) {
                        val userInvitations = response.body()!!
                        Log.d(TAG, "Nhận được ${userInvitations.size} lời mời gửi đến người dùng")

                        for (serverInvitation in userInvitations) {
                            val existingInvitation = teamInvitationDao.getInvitationByServerId(serverInvitation.id.toString())
                            Log.d(TAG, "Kiểm tra lời mời người dùng ${serverInvitation.id}: ${if (existingInvitation != null) "đã tồn tại" else "mới"}")

                            if (existingInvitation == null) {
                                // New invitation from server
                                val newInvitation = serverInvitation.toEntity()
                                teamInvitationDao.insertInvitation(newInvitation)
                                Log.d(TAG, "Đã thêm lời mời người dùng mới vào cơ sở dữ liệu cục bộ: ${serverInvitation.id}")
                            } else if (existingInvitation.syncStatus == "synced") {
                                // Update existing invitation if it's synced (not pending changes)
                                val updatedInvitation = serverInvitation.toEntity(existingInvitation)
                                teamInvitationDao.updateInvitation(updatedInvitation)
                                Log.d(TAG, "Đã cập nhật lời mời người dùng hiện có: ${serverInvitation.id}")
                            }
                        }
                    } else {
                        val errorBody = response.errorBody()?.string()
                        Log.e(TAG, "Lỗi khi lấy lời mời người dùng từ server: ${response.code()}, Error: $errorBody")
                    }
                }
            } catch (e: Exception) {
                Log.e(TAG, "Lỗi khi lấy lời mời người dùng", e)
            }
