...
interface TeamDao {
    ...
    @Query("SELECT * FROM teams")
    fun getAllTeamsSync(): List<TeamEntity>
    ...
    // Debug query để kiểm tra teams và members riêng biệt
    @Query("SELECT * FROM teams")
    fun getAllTeamsDebug(): Flow<List<TeamEntity>>

    @Query("SELECT * FROM team_members WHERE userId = :userId")
    suspend fun getTeamMembersForUserDebug(userId: String): List<com.example.taskapplication.data.database.entities.TeamMemberEntity>
