        ...
        try {
            ...
            try {
                ...
                if (!email.isNullOrEmpty()) {
                    ...
                    try {
                        // Thử gọi API getUserInvitations() - endpoint này chưa được triển khai trên server
                        // Sẽ được thay thế bằng endpoint thực khi backend triển khai
                        ...
                        if (response.isSuccessful && response.body() != null) {
                            ...
                            // Xử lý khi API trả về lỗi (404 Not Found)
                            val errorBody = response.errorBody()?.string()
                            Log.w(TAG, "⚠️ [THEO DÕI] API getUserInvitations() chưa được triển khai: ${response.code()}")
                            Log.d(TAG, "ℹ️ [THEO DÕI] Sử dụng phương pháp thay thế để lấy lời mời người dùng")

                            // Phương pháp thay thế: Lấy lời mời từ cơ sở dữ liệu cục bộ
                            // Lưu ý: Điều này sẽ không đồng bộ lời mời mới từ server, nhưng sẽ tránh lỗi
                            val pendingInvitations = teamInvitationDao.getInvitationsByEmail(email)
                            Log.d(TAG, "ℹ️ [THEO DÕI] Tìm thấy ${pendingInvitations.size} lời mời cục bộ cho email $email")
                        }
                    } catch (e: Exception) {
                        // Xử lý ngoại lệ khi gọi API
                        Log.w(TAG, "⚠️ [THEO DÕI] Không thể gọi API getUserInvitations(): ${e.message}")
                        Log.d(TAG, "ℹ️ [THEO DÕI] Sử dụng phương pháp thay thế để lấy lời mời người dùng")
