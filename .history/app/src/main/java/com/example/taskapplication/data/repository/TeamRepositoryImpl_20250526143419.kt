            ...
            for (member in membersToCreate) {
                try {
                    ...
                    val response = if (false) { // Disable numeric endpoint temporarily
                        ...
                        Log.d(TAG, "🔄 [API] TEMP FIX: Using UUID endpoint: POST /teams/${team.uuid}/invite")
                        Log.d(TAG, "🔄 [API] Team UUID: ${team.uuid}")
                        apiService.sendInvitationByUuid(
                            teamUuid = team.uuid,  // Use team.uuid instead of serverId
                            request = invitationRequest
                        )
                    }
