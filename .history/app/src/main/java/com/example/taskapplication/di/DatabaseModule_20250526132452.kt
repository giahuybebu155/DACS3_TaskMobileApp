...
object DatabaseModule {
    ...
    fun provideAppDatabase(
        ...
        return Room.databaseBuilder(
            ...
            "task_app_database_v14" // Cập nhật database name để match với AppDatabase.kt
        )
        ...
        .addMigrations(
            ...
            AppDatabase.MIGRATION_13_14  // Thêm migration mới
        )
     ...
     * Chỉ xóa các cơ sở dữ liệu cũ khi ứng dụng được cài đặt lần đầu
     */
    private fun deleteOldDatabases(context: Context) {
        // Kiểm tra xem cơ sở dữ liệu hiện tại đã tồn tại chưa
        val currentDbFile = context.getDatabasePath("task_app_database_v13.db")

        // Nếu cơ sở dữ liệu hiện tại đã tồn tại, không xóa bất kỳ cơ sở dữ liệu nào
        if (currentDbFile.exists()) {
            Log.d("DatabaseModule", "Current database exists, skipping deletion of old databases")
            return
        }

        // Chỉ xóa các cơ sở dữ liệu cũ khi cơ sở dữ liệu hiện tại chưa tồn tại
