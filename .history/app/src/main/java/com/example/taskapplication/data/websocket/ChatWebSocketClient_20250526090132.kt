...
class ChatWebSocketClient @Inject constructor(
    ...
    fun connect(token: String) {
        ...
        // Kiểm tra xem server WebSocket có đang chạy không (fast fail)
        try {
            Log.d(TAG, "🔌 [WS_CHECK] Checking WebSocket server connectivity...")
            val socket = java.net.Socket()
            val socketAddress = java.net.InetSocketAddress("10.0.2.2", 8080)
            socket.connect(socketAddress, 1000) // 1 second timeout - fast fail
            socket.close()
            Log.d(TAG, "✅ [WS_CHECK] WebSocket server is reachable")
        } catch (e: Exception) {
            Log.e(TAG, "❌ [WS_CHECK] WebSocket server is not reachable:")
            Log.e(TAG, "❌ [WS_CHECK] Exception type: ${e.javaClass.simpleName}")
            Log.e(TAG, "❌ [WS_CHECK] Exception message: ${e.message}")
            Log.e(TAG, "⚠️ [WS_CHECK] App continues in offline mode - no real-time features")
            messageHandler?.onConnectionError(e)
            return
        }

        val wsUrl = "ws://10.0.2.2:8080?token=$token"
