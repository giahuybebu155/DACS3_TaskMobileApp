...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        fun getInstance(context: android.content.Context): AppDatabase {
            return INSTANCE ?: synchronized(this) {
                val instance = androidx.room.Room.databaseBuilder(
                    ...
                    "task_app_database_v14"  // Cập nhật database name
                )
                .addMigrations(
                    MIGRATION_1_2,
                    MIGRATION_2_3,
                    ...
                    MIGRATION_11_12,
                    MIGRATION_12_13,
                    MIGRATION_13_14  // Thêm migration mới
                )
        ...
        val MIGRATION_13_14 = object : Migration(13, 14) {
            override fun migrate(database: SupportSQLiteDatabase) {
                // Schema integrity fix - no actual schema changes needed
                // This migration is just to update the version number to fix the integrity hash mismatch
                // The schema is already correct, we just need to update the version

                // Log migration for debugging
                android.util.Log.d("AppDatabase", "🔄 [MIGRATION] Migrating from version 13 to 14")
                android.util.Log.d("AppDatabase", "🔄 [MIGRATION] This is a schema integrity fix migration")
                android.util.Log.d("AppDatabase", "✅ [MIGRATION] Migration 13->14 completed successfully")
            }
        }
    }
}