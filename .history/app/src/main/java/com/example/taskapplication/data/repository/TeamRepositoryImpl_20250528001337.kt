        ...
        return teamDao.getTeamsForUser(userId)
            .onEach { entities ->
                ...
                if (entities.isEmpty()) {
                    ...
                    // Debug: Kiểm tra tại sao không có teams
                    try {
                        val allTeams = teamDao.getAllTeamsSync()
                        val userMembers = teamMemberDao.getUserTeamMembershipsSync(userId)

                        Log.d(TAG, "🔍 [DEBUG REPO] Tổng số teams trong DB: ${allTeams.size}")
                        Log.d(TAG, "🔍 [DEBUG REPO] Team members cho user $userId: ${userMembers.size}")

                        allTeams.forEach { team ->
                            Log.d(TAG, "🔍 [DEBUG REPO] Team: id=${team.id}, name=${team.name}")
                        }

                        userMembers.forEach { member ->
                            Log.d(TAG, "🔍 [DEBUG REPO] Member: teamId=${member.teamId}, userId=${member.userId}, role=${member.role}")
                        }
                    } catch (e: Exception) {
                        Log.e(TAG, "❌ [DEBUG REPO] Lỗi khi debug: ${e.message}", e)
                    }
                } else {
