...
class TeamRepositoryImpl @Inject constructor(
    ...
    override suspend fun createTeam(team: Team): Result<Team> {
        try {
            ...
            val teamIdForWebSocket = teamEntity.serverId ?: teamEntity.id
            dataStoreManager.saveCurrentTeamId(teamIdForWebSocket)
            Log.d(TAG, "✅ [THEO DÕI] Đã đặt team làm team hiện tại cho WebSocket: $teamIdForWebSocket")
    ...
    override suspend fun inviteUserToTeam(teamId: String, userEmail: String): Result<com.example.taskapplication.domain.model.TeamMember> {
        try {
            ...
            // ❌ REMOVED: Không thêm thành viên vào team khi gửi invitation
            // ✅ CORRECT: Chỉ thêm thành viên khi họ accept invitation
            // User sẽ được thêm vào team trong acceptInvitation() method

            Log.d(TAG, "📝 [THEO DÕI] Tạo invitation cho user: ${user.id} (${user.email})")
            Log.d(TAG, "ℹ️ [THEO DÕI] User sẽ được thêm vào team sau khi accept invitation")
            ...
            // Create placeholder member for return value
            val placeholderMember = com.example.taskapplication.domain.model.TeamMember(
                id = UUID.randomUUID().toString(),
                teamId = teamId,
                userId = user.id,
                role = "member",
            ...
            if (connectionChecker.isNetworkAvailable()) {
                try {
                    ...
                    if (syncedTeam.serverId == null) {
                        ...
                        return handleInvitationResponse(response, placeholderMember)
                    }
    ...
    /**
     * Helper function to handle invitation response from server
     */
    private fun handleInvitationResponse(
        response: retrofit2.Response<com.example.taskapplication.data.api.response.TeamInvitationResponse>,
        placeholderMember: com.example.taskapplication.domain.model.TeamMember
    ): Result<com.example.taskapplication.domain.model.TeamMember> {
        return if (response.isSuccessful && response.body() != null) {
            val serverInvitation = response.body()!!
            Log.d(TAG, "✅ [THEO DÕI] Lời mời đã được gửi thành công lên server với ID: ${serverInvitation.id}")
            Result.success(placeholderMember)
        } else {
            val errorBody = response.errorBody()?.string()
            Log.e(TAG, "❌ [THEO DÕI] Lỗi khi gửi lời mời lên server: ${response.code()}, Error: $errorBody")
            Result.success(placeholderMember) // Still return success since local invitation was created
        }
    }
