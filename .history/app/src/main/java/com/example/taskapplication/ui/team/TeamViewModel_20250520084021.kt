...
class TeamViewModel @Inject constructor(
    ...
    init {
        ...
        viewModelScope.launch {
            try {
                ...
                Log.d(TAG, "Đã đồng bộ nhóm từ server khi khởi tạo ViewModel")

                // Đồng bộ thành viên nhóm
                try {
                    teamRepository.syncTeamMembers()
                    Log.d(TAG, "Đã đồng bộ thành viên nhóm từ server khi khởi tạo ViewModel")
                } catch (e: Exception) {
                    Log.e(TAG, "Lỗi khi đồng bộ thành viên nhóm từ server: ${e.message}", e)
                }
            } catch (e: Exception) {
                Log.e(TAG, "Lỗi khi đồng bộ nhóm từ server: ${e.message}", e)
            }
            ...
            webSocketManager.events.collect { event ->
                when (event) {
                    is ChatEvent.TeamUpdated -> {
                        Log.d(TAG, "Nhận được sự kiện cập nhật nhóm: ${event.teamId}")
                        refreshTeams()
                    }
                    is ChatEvent.TeamMemberAdded -> {
                        Log.d(TAG, "Nhận được sự kiện thêm thành viên: ${event.userId} vào nhóm ${event.teamId}")
                        refreshTeams()
                    }
                    is ChatEvent.TeamMemberRemoved -> {
                        Log.d(TAG, "Nhận được sự kiện xóa thành viên: ${event.userId} khỏi nhóm ${event.teamId}")
                        refreshTeams()
                    }
                    is ChatEvent.TeamMemberRoleChanged -> {
                        Log.d(TAG, "Nhận được sự kiện thay đổi vai trò: ${event.userId} thành ${event.newRole} trong nhóm ${event.teamId}")
                        refreshTeams()
                    }
                    is ChatEvent.TeamInvitation -> {
                        Log.d(TAG, "Nhận được sự kiện lời mời tham gia nhóm: ${event.teamId}")
                        loadPendingInvitationsCount()
                    }
                    is ChatEvent.TeamInvitationAccepted -> {
                        Log.d(TAG, "Nhận được sự kiện chấp nhận lời mời: ${event.userId} đã tham gia nhóm ${event.teamId}")
                        refreshTeams()
                        loadPendingInvitationsCount()
                    }
                    is ChatEvent.TeamInvitationRejected, is ChatEvent.TeamInvitationCancelled -> {
                        Log.d(TAG, "Nhận được sự kiện từ chối/hủy lời mời")
                        loadPendingInvitationsCount()
                    }
                    else -> {
    ...
    fun forceReloadTeams() {
        viewModelScope.launch {
            _teamsState.value = TeamsState.Loading
            println("🔄 [DEBUG UI] Bắt đầu tải lại dữ liệu từ server")

            try {
                // Xóa dữ liệu cục bộ và tải lại từ server
                teamRepository.clearLocalTeamsAndMembers()
                println("✅ [DEBUG UI] Đã xóa dữ liệu cục bộ")

                // Đồng bộ nhóm từ server
                val syncResult = teamRepository.syncTeams()
                if (syncResult.isSuccess) {
                    println("✅ [DEBUG UI] Đồng bộ nhóm từ server thành công")
                } else {
                    println("❌ [DEBUG UI] Đồng bộ nhóm từ server thất bại: ${syncResult.exceptionOrNull()?.message}")
                }

                // Đồng bộ thành viên nhóm
                try {
                    val memberSyncResult = teamRepository.syncTeamMembers()
                    if (memberSyncResult.isSuccess) {
                        println("✅ [DEBUG UI] Đồng bộ thành viên nhóm thành công")
                    } else {
                        println("❌ [DEBUG UI] Đồng bộ thành viên nhóm thất bại: ${memberSyncResult.exceptionOrNull()?.message}")
                    }
                } catch (e: Exception) {
                    println("❌ [DEBUG UI] Lỗi khi đồng bộ thành viên nhóm từ server: ${e.message}")
                }
                ...
                println("❌ [DEBUG UI] Lỗi khi tải lại dữ liệu từ server: ${e.message}")
                e.printStackTrace()
    ...
    /**
     * Fix team display issues by checking and repairing team memberships
     */
    fun fixTeamDisplayIssues() {
        viewModelScope.launch {
            _teamsState.value = TeamsState.Loading
            println("🔧 [DEBUG UI] Bắt đầu sửa lỗi hiển thị nhóm")

            try {
                val userId = dataStoreManager.getCurrentUserId()
                if (userId == null) {
                    println("❌ [DEBUG UI] Không thể sửa lỗi: Người dùng chưa đăng nhập")
                    _teamsState.value = TeamsState.Error("Người dùng chưa đăng nhập")
                    return@launch
                }
                ...
                println("❌ [DEBUG UI] Lỗi khi sửa lỗi hiển thị nhóm: ${e.message}")
                e.printStackTrace()
                _teamsState.value = TeamsState.Error(e.message ?: "Không thể sửa lỗi hiển thị nhóm")
            }
        }
    }
}
