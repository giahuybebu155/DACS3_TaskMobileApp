...
class SyncRepositoryImpl @Inject constructor(
    ...
    override suspend fun schedulePeriodicSync() {
        ...
        val intervalMinutes = 15 // Default interval of 15 minutes
    ...
    override suspend fun syncSelectedItems(
        tasks: List<com.example.taskapplication.data.database.entities.PersonalTaskEntity>,
        subtasks: List<com.example.taskapplication.data.database.entities.SubtaskEntity>
    ): Result<Unit> {
        if (!connectionChecker.isNetworkAvailable()) {
            return Result.failure(IOException("No network connection"))
        }
        ...
        try {
            ...
            val changes = mutableListOf<com.example.taskapplication.data.api.model.ChangeItem>()

            // Thêm các task được chọn
            tasks.forEach { task ->
                when (task.syncStatus) {
                    "pending_create" -> {
                        val taskData = mapOf(
                            ...
                            "status" to if (task.isCompleted) "completed" else "pending",
                            "priority" to task.priority.toString(),
                            "due_date" to (task.dueDate ?: 0L).toString()
                        )
                        changes.add(com.example.taskapplication.data.api.model.ChangeItem(
                            type = "create",
                            model_type = "personal_task",
                            data = taskData,
                            client_id = task.id
                        ))
                    }
            ...
            // Thêm các subtask được chọn
            subtasks.forEach { subtask ->
                when (subtask.syncStatus) {
                    "pending_create" -> {
                        val subtaskData = mapOf(
                            "title" to subtask.title,
                            "is_completed" to subtask.isCompleted.toString(),
                            "task_id" to subtask.taskId,
                            "order" to subtask.order.toString()
                        )
                        changes.add(com.example.taskapplication.data.api.model.ChangeItem(
                            type = "create",
                            model_type = "subtask",
            ...
            // Gửi các thay đổi lên server
            val response = apiService.pushChanges(
                deviceId = deviceId,
                changes = changes
            )
            ...
            // Cập nhật trạng thái đồng bộ của các task
            tasks.forEach { task ->
                if (task.syncStatus != "synced") {
                    val updatedTask = task.copy(
                        syncStatus = "synced",
                        serverId = syncResponse.serverIds[task.id] ?: task.serverId
                    )
                    personalTaskDao.updateTask(updatedTask)
                }
            }

            // Cập nhật trạng thái đồng bộ của các subtask
            subtasks.forEach { subtask ->
                if (subtask.syncStatus != "synced") {
                    val updatedSubtask = subtask.copy(
                        syncStatus = "synced",
                        serverId = syncResponse.serverIds[subtask.id] ?: subtask.serverId
                    )
                    personalTaskDao.updateSubtask(updatedSubtask)
                }
            }

            Log.d(TAG, "Selected items sync completed successfully")
            return Result.success(Unit)
        } catch (e: Exception) {
            Log.e(TAG, "Selected items sync failed", e)
            return Result.failure(e)
        }
    }
}
