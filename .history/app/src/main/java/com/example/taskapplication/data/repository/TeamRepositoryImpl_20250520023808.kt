...
class TeamRepositoryImpl @Inject constructor(
        ...
        try {
            ...
            if (connectionChecker.isNetworkAvailable()) {
                try {
                    ...
                    // Sử dụng UUID của nhóm từ server
    ...
    override suspend fun syncTeams(): Result<Unit> {
        ...
        try {
            ...
            Log.d(TAG, "🔄 [THEO DÕI] Đang lấy các thay đổi từ server")
            val lastSyncTimestamp = dataStoreManager.getLastTeamSyncTimestamp()
            Log.d(TAG, "🔍 [THEO DÕI] Thời điểm đồng bộ cuối cùng: $lastSyncTimestamp")
            ...
            try {
                ...
                if (response.isSuccessful && response.body() != null) {
                    ...
                    Log.d(TAG, "✅ [THEO DÕI] Nhận được phản hồi từ server")

                    // Xử lý các team mới
                    if (syncResponse.data.teams != null) {
                        val createdTeams = syncResponse.data.teams.created ?: emptyList()
                        Log.d(TAG, "🔍 [THEO DÕI] Có ${createdTeams.size} nhóm mới từ server")

                        for (team in createdTeams) {
                            Log.d(TAG, "🔄 [THEO DÕI] Đang thêm nhóm mới từ server: ${team.name} (ID: ${team.id})")
                            teamDao.insertTeam(team.toEntity())
                            Log.d(TAG, "✅ [THEO DÕI] Đã thêm nhóm mới vào cơ sở dữ liệu cục bộ")
                        }

                        // Xử lý các team đã cập nhật
                        val updatedTeams = syncResponse.data.teams.updated ?: emptyList()
                        Log.d(TAG, "🔍 [THEO DÕI] Có ${updatedTeams.size} nhóm đã cập nhật từ server")

                        for (team in updatedTeams) {
                            Log.d(TAG, "🔄 [THEO DÕI] Đang cập nhật nhóm từ server: ${team.name} (ID: ${team.id})")
                            teamDao.updateTeam(team.toEntity())
                            Log.d(TAG, "✅ [THEO DÕI] Đã cập nhật nhóm trong cơ sở dữ liệu cục bộ")
                        }

                        // Xử lý các team đã xóa
                        val deletedTeams = syncResponse.data.teams.deleted ?: emptyList()
                        Log.d(TAG, "🔍 [THEO DÕI] Có ${deletedTeams.size} nhóm đã xóa từ server")

                        for (teamId in deletedTeams) {
                            Log.d(TAG, "🔄 [THEO DÕI] Đang xóa nhóm từ server: ID: $teamId")
                    ...
                    Log.d(TAG, "✅ [THEO DÕI] Đã cập nhật thời điểm đồng bộ: ${syncResponse.meta.syncTimestamp}")
                } else {
                    Log.e(TAG, "❌ [THEO DÕI] Lỗi khi lấy thay đổi từ server: ${response.code()}")
                    val errorBody = response.errorBody()?.string()
                    Log.e(TAG, "❌ [THEO DÕI] Chi tiết lỗi: $errorBody")
                }
            } catch (e: Exception) {
                Log.e(TAG, "❌ [THEO DÕI] Lỗi khi lấy thay đổi từ server: ${e.message}", e)
            }
