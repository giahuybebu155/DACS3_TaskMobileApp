    ...
    Scaffold(
        topBar = {
            TopAppBar(
                ...
                actions = {
                    // Hiển thị biểu tượng đồng bộ
                    IconButton(
                        onClick = { viewModel.refreshInvitations() },
                        enabled = connectionState == ConnectionState.CONNECTED
                    ) {
                        Icon(
                            imageVector = Icons.Default.Sync,
                            contentDescription = "Sync Invitations",
        ...
        Column(modifier = Modifier.padding(paddingValues)) {
            // Hiển thị thông báo chỉ khi có lỗi kết nối thực sự (ERROR), không hiển thị khi DISCONNECTED
            if (connectionState == ConnectionState.ERROR) {
                Snackbar(
                    modifier = Modifier.padding(16.dp),
                    action = {
                        TextButton(onClick = { viewModel.reconnectWebSocket() }) {
                            Text("Kết nối lại")
                        }
                    },
                    ...
                    Text("Mất kết nối với server. Một số tính năng có thể không hoạt động.")
                }
            }
