...
class TeamInvitationRepositoryImpl @Inject constructor(
                    ...
                    try {
                        ...
                        if (response.isSuccessful && response.body() != null) {
                            ...
                            for (serverInvitation in userInvitations) {
                                Log.d(TAG, "🔍 [THEO DÕI] Xử lý lời mời người dùng từ server: ID=${serverInvitation.id}, status=${serverInvitation.status}, teamId=${serverInvitation.teamId}")
                                val existingInvitation = teamInvitationDao.getInvitationByServerId(serverInvitation.id.toString())
                                ...
                                if (existingInvitation == null) {
                                    ...
                                    val newInvitation = serverInvitation.toEntity()
                                    Log.d(TAG, "📝 [THEO DÕI] Tạo entity lời mời người dùng mới: ${newInvitation.id}, email=${newInvitation.email}, teamId=${newInvitation.teamId}")
                                    teamInvitationDao.insertInvitation(newInvitation)
     ...
     * Hiển thị thông báo cho lời mời mới
     */
    private fun showNotificationForNewInvitation(invitation: com.example.taskapplication.data.database.entities.TeamInvitationEntity) {
        try {
            Log.d(TAG, "🔔 [THEO DÕI] Hiển thị thông báo cho lời mời mới: ${invitation.id}")

            // Tạo kênh thông báo nếu chưa có
            val notificationManager = android.content.Context.NOTIFICATION_SERVICE.let {
                context.getSystemService(it) as android.app.NotificationManager
            }
            ...
            val intent = android.content.Intent(context, com.example.taskapplication.ui.MainActivity::class.java).apply {
            ...
            // Tạo thông báo
            val notification = androidx.core.app.NotificationCompat.Builder(context, "team_invitations")
