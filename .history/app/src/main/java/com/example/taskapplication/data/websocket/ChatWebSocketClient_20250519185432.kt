...
class ChatWebSocketClient @Inject constructor(
    ...
    fun connect(token: String) {
        ...
        // Kiểm tra xem server WebSocket có đang chạy không
        try {
            val socket = java.net.Socket()
            val socketAddress = java.net.InetSocketAddress("10.0.2.2", 8080)
            socket.connect(socketAddress, 3000) // 3 seconds timeout
            socket.close()
            Log.d(TAG, "WebSocket server is reachable")
        } catch (e: Exception) {
            Log.e(TAG, "WebSocket server is not reachable", e)
            messageHandler?.onConnectionError(e)
            return
        }

        val wsUrl = "ws://10.0.2.2:8080/ws?token=$token"
