...
class TeamRepositoryImpl @Inject constructor(
    ...
    override fun getTeamsForUser(userId: String): Flow<List<Team>> {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            val memberCount = teamMemberDao.getUserTeamMembershipsSync(userId).size
            Log.d(TAG, "🔍 [DEBUG REPO] Số lượng nhóm mà userId $userId là thành viên: $memberCount")
    ...
    override suspend fun inviteUserToTeam(teamId: String, userEmail: String): Result<com.example.taskapplication.domain.model.TeamMember> {
        try {
            ...
            val isManager = teamMemberDao.isUserAdminOfTeam(teamId, currentUserId) // Vẫn sử dụng phương thức isUserAdminOfTeam nhưng hiểu là kiểm tra vai trò manager
            if (!isManager) {
                Log.e(TAG, "❌ [THEO DÕI] Người dùng không phải là manager của nhóm")
