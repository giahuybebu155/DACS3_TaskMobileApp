...
interface TeamRoleHistoryDao {
    ...
    /**
     * Get role history entries that need to be synced (suspend function)
     */
    @Query("SELECT * FROM team_role_history WHERE syncStatus = 'pending'")
    suspend fun getPendingRoleHistory(): List<TeamRoleHistoryEntity>
     ...
     * Update sync status for a role history entry
     */
    @Query("UPDATE team_role_history SET syncStatus = :status WHERE id = :id")
    suspend fun updateRoleHistorySyncStatus(id: Long, status: String)
